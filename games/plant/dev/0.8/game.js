...
function compute_node_movement() {
	...
	for (d = nodes_by_depth.length - 1; d >= 0; d--) {
		...
		for (i = 0; i < nodes.length; i++) {
			...
			hooke = - (1 - 1 / (1 + node.weight)) * angular_displacement; // can experiment with different spring constants
			// hooke = -0.8*angular_displacement;
			// hooke -= 0.5 * node.dtheta;
			var ddtheta = (torque.mag*torque.dir + hooke) /* (node.length * node.cog.m)*/ /*(0.1 + 0.1*Math.random())*/ - 0.1 * node.dtheta;
			node.dtheta += ddtheta;
