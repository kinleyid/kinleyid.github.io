...
function add_node(node, theta, length) {
	...
	plant_stats.sugar -= sugar_costs.node * length/max_segment_length;
}
...
var colours = {
	node: 'rgb(150, 75, 0)',
	leaf_diaph: 'rgba(0, 200, 0, 0.5)',
	leaf_opaque: 'rgb(0, 200, 0)',
}
...
function draw_info() {
	...
	var lines = [
		...
		'New leaf: "L"',
		'Drag view: "D"',
		'Sugar: ' + Math.floor(plant_stats.sugar) + '.' + (Math.floor(10*plant_stats.sugar) - 10*Math.floor(plant_stats.sugar))];
	var i;
...
function draw_all_leaves() {
	ctx.fillStyle = colours.leaf_diaph;
...
function draw_cursor() {
	if (game_mode == 'default') {
		...
		ctx.strokeStyle = colours.leaf_opaque;
		ctx.beginPath();
		ctx.arc(mouse.x, mouse.y, 2, 0, 2 * Math.PI);
		ctx.stroke();
		ctx.strokeStyle = 'black';
