...
function compute_centre_of_gravity(node) {
	...
	var m = node.length / max_segment_length;
	var i;
	for (i = 0; i < node.children.length; i++) {
		m += node.children[i].cog.m;
	}
	for (dim in cog) {
		var d = (node[dim] + node.parent[dim])/2;
		var i;
		for (i = 0; i < node.children.length; i++) {
...
function compute_torque(node) {
	...
	var F = 0.0005 * node.cog.m; // Can experiment with different gravitational constants
...
function compute_node_movement() {
	...
	for (d = nodes_by_depth.length - 1; d >= 0; d--) {
		...
		for (i = 0; i < nodes.length; i++) {
			...
			hooke = -0.6 * Math.tanh(node.theta.rel.curr - node.theta.rel.ur) - 0.5*node.dtheta // can experiment with different spring constants
			var ddtheta = torque.mag*torque.dir + hooke;
			node.dtheta += ddtheta;
			node.theta.rel.curr += nodes[i].dtheta;
		}
	}
}
