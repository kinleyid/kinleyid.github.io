...
function compute_node_movement() {
	...
	for (d = nodes_by_depth.length - 1; d >= 0; d--) {
		...
		for (i = 0; i < nodes.length; i++) {
			...
			hooke = - 0.8*node.weight * Math.sign(angular_displacement)*Math.abs(angular_displacement); // can experiment with different spring constants
...
function respond_to_mouseup(click) {
	if (game_mode == 'new node: select parent') {
		...
		if (parent) {
			// Win condition!
			var elapased_time = (performance.now() - start_time)/1000/60;
			flowering_node = parent;
			plant_stats.energy -= energy_costs.flower;
			all_bugs = [];
			// Add win note
			var info_div = document.getElementById('game-info');
			info_div.innerHTML = '<div style="text-align: center;"><h2>Win!</h2>Time: ' + elapased_time.toFixed(2) + ' minutes<br>Reload page to play again</div>'
		}
	}
}
